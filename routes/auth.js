  ...
  new GoogleStrategy(
    {
      ...
      callbackURL: "https://tourprism-backend-w5c1.onrender.com/auth/google/callback",
    },
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (parentUser) {
      console.log('Found parent user for collaborator email:', email);
      
      // Find the matching collaborator
      const collaborator = parentUser.collaborators.find(c => c.email === email);
      
      if (collaborator) {
        console.log('Found collaborator:', JSON.stringify({
          email: collaborator.email,
          role: collaborator.role,
          status: collaborator.status,
          hasPassword: !!collaborator.password,
          passwordLength: collaborator.password ? collaborator.password.length : 0
        }));
        
        // Modified password validation check - just check if the password field exists at all
        if (!collaborator.password) {
          console.log('Collaborator has no password');
        ...
        try {
          console.log('Attempting to verify collaborator password');
          console.log('Collaborator password hash length:', collaborator.password ? collaborator.password.length : 0);
          ...
          console.log('Password verification result:', isCollabMatch);
          ...
          if (isCollabMatch) {
            ...
            if (collaborator.status !== 'active') {
              console.log(`Collaborator status check failed: Status is '${collaborator.status}' instead of 'active'`);
            ...
            console.log('Collaborator login successful, generating token for collaborator with status:', collaborator.status);
            ...
            console.log('Password did not match for collaborator');
            return res.status(400).json({ message: "Invalid credentials - incorrect password" });
          }
        } catch (bcryptError) {
          console.error("Password comparison error:", bcryptError);
          console.error("Password hash:", collaborator.password);
          console.error("Input password length:", password ? password.length : 0);
