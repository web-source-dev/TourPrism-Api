...
router.put('/company-info', authenticate, canEditProfile, async (req, res) => {
  try {
    ...
    console.log('Received company update:', { companyName, companyType, mainOperatingRegions });
    ...
    console.log('Before update - user.company:', user.company);

    // Update company fields
    if (companyName !== undefined) {
      console.log(`Setting company name from "${user.company.name}" to "${companyName}"`);
      ...
      try {
        ...
        if (!existingCompany && companyName.trim()) {
          ...
          console.log(`Added new company name to suggestions: ${companyName}`);
        }
      } catch (error) {
    ...
    if (mainOperatingRegions && Array.isArray(mainOperatingRegions)) {
      ...
      if (isNewFormat) {
        ...
        console.log(`Updated operating regions with coordinates. Count: ${validRegions.length}`);
      } else {
        ...
        console.log(`Converted legacy operating regions. Count: ${convertedRegions.length}`);
      }
    }
    ...
    const updatedUser = await User.findById(req.userId).select('-password -otp -otpExpiry -resetPasswordToken -resetPasswordExpiry');
    console.log('After update - user.company:', updatedUser.company);
