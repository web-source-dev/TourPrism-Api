...
const sendSubscriptionVerificationEmail = async (email, otp) => {
    const mailOptions = {
      from: process.env.EMAIL_FROM || process.env.EMAIL_USER || "no-reply@tourprism.com",
      to: email,
      subject: 'Verify Your Email - TourPrism Disruption Alerts',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: white; padding: 40px 30px; border-radius: 0 0 10px 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            ...
            <p style="color: #666; line-height: 1.6; margin-bottom: 30px;">
              Thank you for subscribing to TourPrism disruption alerts! We'll keep you informed about travel disruptions that could impact your guests.
            </p>
      ...
      `,
      emailType: 'subscription_verification'
    };

    try {
      await transporter.sendMail(mailOptions);
      console.log(`Subscription verification email sent successfully to ${email}`);
      return true;
    } catch (error) {
...
module.exports = sendSubscriptionVerificationEmail;
